    ...
    try {
      if (searchInp.current.value) {
        ...
        const { data } = await axios.get(
          `${API_URL}?query=${searchInp.current.value
          }&page=${pge}&per_page=${per_page}&client_id=${import.meta.env.VITE_API_KEY
          }`
        );
        setImg(data.results);
  ...
  return (
    <>
      <div>
        ...
        <div className="flex gap-5 flex-wrap justify-center mt-10 h-screen">
          {img.map((pic) => {
            return (
