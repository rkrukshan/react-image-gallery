...
export default function Gallery() {
  ...
  return (
    <div>
      ...
      <button className="bg-blue-500 rounded-xl text-white px-5 py-53 m-10">
        {pge > 1 && <button onClick={()=>{setPge(pge - 1)}}>Previous</button>}
        {pge < tpage && <button onClick={()=>{setPge(pge + 1)}}>Next</button>}
      </button>
    </div>
  );
}
